...
export const apiClient = apiJobCategory;
export type RecordType = JobCategoryDTO;
...
export const PageProvider: React.FC<{}> = (props: Props) => {
  ...
  const fetchAll = React.useCallback(async () => {
    try {
      ...
      const data = await apiClient.jobCategory_GetAll();
      setListData(data);
    } catch (err) {
      console.error(err);
  ...
  const onCreate = React.useCallback(
    async (record: RecordType) => {
      try {
        const newRecord = await apiClient.jobCategory_Create(record);
  ...
  const onUpdate = React.useCallback(
    async (record: RecordType) => {
      try {
        ...
        await apiClient.jobCategory_Update(updatedRecord.id!, updatedRecord);
        setListData((data) =>
  ...
  const onDelete = React.useCallback(async (recordId: number) => {
    try {
      await apiClient.jobCategory_Delete(recordId);
