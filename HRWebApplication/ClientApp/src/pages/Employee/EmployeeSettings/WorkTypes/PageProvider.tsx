...
import { apiWorkType } from 'services/ApiClient.singleton';
import { CreateUpdateModal } from './CreateUpdateModal';

export const apiClient = apiWorkType;
export type RecordType = WorkTypeDTO;
...
export const PageProvider: React.FC<{}> = (props: Props) => {
  ...
  const fetchAll = React.useCallback(async () => {
    try {
      ...
      const data = await apiClient.workType_GetAll();
      setListData(data);
    } catch (err) {
      console.error(err);
  ...
  const onCreate = React.useCallback(
    async (record: RecordType) => {
      try {
        const newRecord = await apiClient.workType_Create(record);
  ...
  const onUpdate = React.useCallback(
    async (record: RecordType) => {
      try {
        ...
        await apiClient.workType_Update(updatedRecord.id!, updatedRecord);
        setListData((data) =>
  ...
  const onDelete = React.useCallback(async (recordId: number) => {
    try {
      await apiClient.workType_Delete(recordId);
